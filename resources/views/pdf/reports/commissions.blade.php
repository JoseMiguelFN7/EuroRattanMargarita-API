...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Encargos</title>
    <style>
...
<body>
    ...
    <main>
        <h2>Reporte de Encargos</h2>

        <table>
            <thead>
                <tr>
                    <th width="25%" class="text-left">CLIENTE</th>
                    <th width="15%" class="text-left">CÓDIGO ENCARGO</th>
                    <th width="20%">ESTATUS</th>
                    <th width="15%">FECHA</th>
                    <th width="25%" class="text-left">ORDEN ASOCIADA</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($commissions as $commission)
                    <tr>
                        <td class="text-left">
                            <strong>{{ $commission->user->name ?? 'Usuario Desconocido' }}</strong><br>
                            <span style="font-size: 9px; color: #888;">
                                {{ $commission->user->email ?? 'N/A' }}
                            </span>
                        </td>

                        <td class="text-left">{{ $commission->code }}</td>

                        <td>
                            @php
                                $rawStatus = strtolower($commission->status);
                        ...
                        <td>
                            ...
                            <span style="font-size: 9px; color: #666;">{{ strtolower($commission->created_at->format('h:i A')) }}</span>
                        </td>

                        <td class="text-left">
                            @if($commission->order)
                                <strong>{{ $commission->order->code }}</strong>
                            @else
                                <span style="font-size: 9px; color: #999;">SIN ORDEN</span>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 25px; color: #666;">
                            No se encontraron encargos registrados o que coincidan con la búsqueda.
                        </td>
                    </tr>
                @endforelse
            </tbody>
